...
- Deviation from spec, intentional: `middleman_get_stack_context` uses the existing per-PR route `GET /pulls/.../stack` instead of scanning `GET /stacks` — same semantics, one existing route, less data transferred. The spec's `GET /stacks` remains available and unused.
