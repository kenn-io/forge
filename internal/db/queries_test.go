...
func TestUpsertRepoByProviderIDResetsRecreatedRepositoryAtSamePath(t *testing.T) {
	...
	assert.NotNil(retiredWorkspace.RetiredAt)
	...
	activeWorkspaces, err := d.ListActiveWorkspaces(ctx)
	require.NoError(err)
	require.Len(activeWorkspaces, 1)
	assert.Equal("new-incarnation-workspace", activeWorkspaces[0].ID)
	allWorkspaces, err := d.ListWorkspaces(ctx)
	require.NoError(err)
	assert.Len(allWorkspaces, 2)
