...
- Startup reconciliation reopens completed GitHub archive lookups when a current close lacks an authored close event or a merge lacks its actor, changed-file count, or merge SHA. Canonical hydration fills those details; no migration invents provider data. (`internal/db/queries_archive.go::RequeueArchiveLifecycleDetails`)
...
- Detailed reports use schema `middleman-archive-report/1` and half-open UTC windows. They expose opened items, current close/merge lifecycle projections, comments, reviews, and inline comments; a reopened issue has no close row. Lifecycle actors come from the newest authored matching event, while merge metrics come from the normalized merge-request row. (`internal/db/queries_archive_report.go::archiveReportActivityQuery`, `internal/archive/report/model.go`)
