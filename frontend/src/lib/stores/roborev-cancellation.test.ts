...
import { createReviewStore } from "./roborev/review.svelte.js";
...
describe("Roborev request cancellation", () => {
  it("aborts stale review transport when another job becomes authoritative", async () => {
    ...
    const get = vi.fn((path: string, options?: { params?: { query?: { job_id?: number } }; signal?: AbortSignal }) => {
      ...
      if (path === "/api/review") {
        return Promise.resolve({
          data: { id: 7, job_id: 43, output: "current review", closed: false },
          error: undefined,
        });
      }
  ...
  it("keeps an accepted close bound to its submitted job when a queued comment settles after navigation", async () => {
    ...
    const get = vi.fn((path: string, options?: { params?: { query?: { job_id?: number; id?: number } } }) => {
      const jobID = options?.params?.query?.job_id ?? options?.params?.query?.id ?? 42;
      if (path === "/api/review") {
        return Promise.resolve({
          data: {
            id: jobID,
    ...
    const post = vi.fn((path: string, options: { body: { job_id: number; closed?: boolean } }) => {
      if (path === "/api/comment") return commentResponse.promise;
      const body = { job_id: options.body.job_id, closed: options.body.closed ?? false };
  ...
  it("reconciles a lost comment response without replaying the comment", async () => {
    ...
    const get = vi.fn((path: string) => {
      ...
      if (path === "/api/comments") {
        return Promise.resolve({ data: { responses: commentCommitted ? [acceptedComment] : [] }, error: undefined });
      }
      return Promise.resolve({
        data: { jobs: [], has_more: false, stats: { done: 0, closed: 0, open: 0 } },
        error: undefined,
