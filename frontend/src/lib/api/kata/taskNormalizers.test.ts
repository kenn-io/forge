...
describe("kata task normalizers", () => {
  ...
  test("maps project stats to sidebar open counts and parses project metadata", () => {
    const project = normalizeKataProject({
      id: 2,
  ...
  test("preserves issue identity, relationship, metadata, recurrence, and date fields", () => {
    const issue = normalizeKataTaskSummary({
      id: 101,
      uid: "issue-pay-rent",
      project_id: 2,
      project_uid: "project-finances",
  ...
  test("derives parent short id from expanded parent refs", () => {
    ...
    expect(issue.parent_short_id).toBe("parent");
    expect(issue.parent).toEqual({ uid: "issue-parent", short_id: "parent" });
  });

  test("keeps absent relationship fields absent for sparse issue rows", () => {
    const sparse = normalizeKataTaskSummary({ uid: "issue-sparse", short_id: "sparse" });
    const explicitEmpty = normalizeKataTaskSummary({
      uid: "issue-empty",
      short_id: "empty",
  ...
  test("falls back to empty metadata objects for absent, empty, and invalid metadata strings", () => {
    expect(normalizeKataTaskSummary({ metadata: "", labels: null }).metadata).toEqual({});
    expect(normalizeKataTaskDetail({ issue: { metadata: "{broken" } }).issue.metadata).toEqual({});
    expect(normalizeKataProject({ metadata: null }).metadata).toEqual({});
  });
  ...
  test("normalizes event payload JSON strings and preserves hub cursor fields", () => {
    const event = normalizeKataEventEnvelope({
      event_id: 5,
