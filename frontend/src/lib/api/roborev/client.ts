...
export function decodeRoborevNdjson<S extends Schema.ConstraintDecoder<unknown>>(
  response: Response,
  schema: S,
  operation: string,
  ...
  return Stream.suspend(() => {
    ...
    const decodeLine = (line: string): Stream.Stream<S["Type"], RoborevStreamError> => {
      let input: unknown;
      try {
        ...
        return invalidRecordPolicy === "skip"
          ? Stream.empty
          : Stream.fail(RoborevStreamError.make({ operation, retryable: true, cause }));
      }
      ...
      return invalidRecordPolicy === "skip"
        ...
        : Stream.fail(
            RoborevStreamError.make({
              operation,
              retryable: true,
              cause: new Error("Roborev NDJSON stream returned an invalid record"),
            }),
          );
    };
    const decodeLines = (lines: ReadonlyArray<string>) => Stream.fromIterable(lines).pipe(Stream.flatMap(decodeLine));
    const values = responseByteStream(response, operation).pipe(
      Stream.mapError((cause) => RoborevStreamError.make({ operation, retryable: true, cause })),
      Stream.decodeText(),
...
> {
  ...
  return Stream.unwrap(
    Effect.gen(function* () {
      const response = yield* Effect.acquireRelease(
        openStreamingResponse("open Roborev event stream", url, {
          ...
          Effect.mapError((cause) =>
            RoborevStreamError.make({ operation: "open Roborev event stream", retryable: true, cause }),
          ),
        ),
        releaseResponseBody,
        { interruptible: true },
      ...
      if (!response.ok) {
        return Stream.fail(
          RoborevStreamError.make({
            ...
            cause: new Error(`Roborev event stream returned ${response.status}`),
          }),
        );
      }
      ...
      return Stream.concat(
        Stream.succeed<RoborevEvent | RoborevStreamOpened>(RoborevStreamOpened.make({ opened: true })),
        Stream.concat(
          decodeRoborevNdjson(response, RoborevEvent, "read Roborev event stream", "fail"),
          Stream.fail(
            RoborevStreamError.make({
...
export const loadRoborevJobOutput = Effect.fn("RoborevClient.loadJobOutput")(function* (
  ...
  return yield* Effect.scoped(
    Effect.gen(function* () {
      const response = yield* Effect.acquireRelease(
        openStreamingResponse("load Roborev job output", jobOutputUrl(baseUrl, jobID, false)).pipe(
          Effect.mapError((cause) =>
            RoborevStreamError.make({ operation: "load Roborev job output", retryable: true, cause }),
          ),
        ),
        releaseResponseBody,
        { interruptible: true },
      ...
      if (!response.ok) {
        return yield* Effect.fail(
          RoborevStreamError.make({
            ...
            retryable: response.status === 408 || response.status === 429 || response.status >= 500,
            cause: new Error(`Roborev job output returned ${response.status}`),
          }),
        );
      }
      const input = yield* Effect.tryPromise({
        try: () => response.json(),
        catch: (cause) => RoborevStreamError.make({ operation: "decode Roborev job output", retryable: false, cause }),
      });
      return yield* Schema.decodeUnknownEffect(RoborevJobOutputSnapshot)(input).pipe(
        Effect.mapError((cause) =>
          RoborevStreamError.make({ operation: "decode Roborev job output", retryable: false, cause }),
        ),
      );
    }),
  );
...
export function roborevJobOutputStream(
  baseUrl: string,
  jobID: number,
): Stream.Stream<RoborevLogLinePayload, RoborevStreamError, import("../../browser/streaming-fetch.js").StreamingFetch> {
  return Stream.unwrap(
    Effect.gen(function* () {
      const response = yield* Effect.acquireRelease(
        openStreamingResponse("stream Roborev job output", jobOutputUrl(baseUrl, jobID, true)).pipe(
          Effect.mapError((cause) =>
            RoborevStreamError.make({ operation: "stream Roborev job output", retryable: true, cause }),
          ),
        ),
        releaseResponseBody,
        { interruptible: true },
      ...
      return decodeRoborevNdjson(response, RoborevLogLinePayload, "read Roborev job output", "skip");
    }),
  );
}
