...
func TestRepoBrowserReadContinuesWhileStagingRefreshFetches(t *testing.T) {
	require := require.New(t)
	assert := assert.New(t)
	mgr, repo, initialSHA, _ := setupRepoBrowserPendingUpdateTest(t)
	...
	go func() {
		refreshDone <- restarted.RefreshRepoBrowserClone(t.Context(), repo)
	}()
	<-routes.started

	readWaiting := make(chan struct{})
	var waitingOnce sync.Once
	restarted.repoBrowserReadWaitingForTest = func(string) {
	...
	require.NoError(err)
	assert.NotEqual(initialSHA, resolved.SHA)
}
...
func TestRepoBrowserPublicationWaitsForAdmittedReader(t *testing.T) {
	...
	// Only the first admitted reader parks; sync.Once would also block the
	// refresh's own staging read until the parked reader returns.
	...
	require.Eventually(func() bool {
		...
		return barrier.waitingWriters > 0
	}, 10*time.Second, 5*time.Millisecond, "publication never queued behind the admitted reader")
	close(releaseRead)
...
// setupRepoBrowserPendingUpdateTest publishes a browser clone and then pushes
// an upstream commit the clone has not fetched yet.
func setupRepoBrowserPendingUpdateTest(
	t *testing.T,
	...
	commitTestRun(t, work, "git", "commit", "-m", "replace route with repository B")
	commitTestRun(t, work, "git", "push", "origin", "main")
	return mgr, repo, initialSHA, work
}
