...
test.describe("terminal state icons", () => {
  ...
  test("refresh preserves workspace diff target and commit selection", async ({ page }) => {
    ...
    await page.goto("/terminal/ws-123");
    await page.locator(".sidebar-toggle-btn", { hasText: "Diff" }).click();
...
test.describe("sidebar toggle behavior", () => {
  ...
  test("segmented control visible in terminal header", async ({ page }) => {
    ...
    await expect(segControl.locator(".sidebar-toggle-btn", { hasText: "PR" })).toBeVisible();
    await expect(
      segControl.locator(".sidebar-toggle-btn", {
        hasText: "Reviews",
      }),
    ).toBeVisible();
  });
  ...
  test("clicking Reviews switches tab without closing", async ({ page }) => {
    ...
    const reviewsBtn = page.locator(".sidebar-toggle-btn", {
      hasText: "Reviews",
    });
...
test.describe("sidebar persistence", () => {
  ...
  test("sidebar tab persists across reload", async ({ page }) => {
    ...
    // Reload
    await page.reload();
    const reviewsBtnAfter = page.locator(".sidebar-toggle-btn", {
      hasText: "Reviews",
    });
  ...
  test("sidebar width persists after resize and reload", async ({ page }) => {
    ...
    // Open sidebar
    await page.locator(".sidebar-toggle-btn", { hasText: "PR" }).click();
    await expect(page.locator(".right-sidebar")).toBeVisible();
...
test.describe("sidebar PR tab", () => {
  ...
  test("workspace without associated PR hides malformed PR tab", async ({ page }) => {
    ...
    await expect(page.locator(".sidebar-toggle-btn", { hasText: "PR" })).toHaveCount(0);
  });
});
...
test.describe("workspace list bubble opens right sidebar", () => {
  ...
  test("clicking PR bubble opens PR tab in the right sidebar", async ({ page }) => {
    ...
    await expect(page.locator(".right-sidebar")).toBeVisible();
    await expect(page.locator(".sidebar-toggle-btn", { hasText: "PR" })).toHaveClass(/\bactive\b/);
  });
  ...
  test("clicking issue bubble opens Issue tab for issue workspace", async ({ page }) => {
    ...
    await expect(page.locator(".right-sidebar")).toBeVisible();
    await expect(page.locator(".sidebar-toggle-btn", { hasText: "Issue" })).toHaveClass(/\bactive\b/);
  });
...
test.describe("issue workspace sidebar", () => {
  ...
  test("issue workspaces show an Issue segment instead of PR and Reviews when no PR is linked", async ({ page }) => {
    ...
    await expect(page.locator(".sidebar-toggle-btn", { hasText: "Issue" })).toBeVisible();
    await expect(page.locator(".sidebar-toggle-btn", { hasText: "PR" })).toHaveCount(0);
    await expect(page.locator(".sidebar-toggle-btn", { hasText: "Reviews" })).toHaveCount(0);
  });
  ...
  test("manual refresh rechecks issue workspace PR association", async ({ page }) => {
    ...
    const refreshButton = page.locator(".header-right .sidebar-toggle-group + .header-icon-btn");
  ...
  test("issue segment opens issue detail for issue-backed workspaces", async ({ page }) => {
    ...
    await page.locator(".sidebar-toggle-btn", { hasText: "Issue" }).click();
  ...
  test("issue workspace gains PR tab after workspace_status refetch and keeps manual PR selection", async ({
    ...
    const issueButton = page.locator(".sidebar-toggle-btn", { hasText: "Issue" });
    const prButton = page.locator(".sidebar-toggle-btn", { hasText: "PR" });
...
test.describe("sidebar Reviews tab", () => {
  ...
  test("Reviews tab preserves a daemon version that already starts with v", async ({ page }) => {
    ...
    await page.locator(".sidebar-toggle-btn", { hasText: "Reviews" }).click();
    await expect(page.locator(".right-sidebar")).toBeVisible();
