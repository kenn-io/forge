...
func TestQueueArchivePromptByIdentityTargetsCurrentRouteOccupant(t *testing.T) {
	...
	maintenanceSucceededAt := now.Add(2 * time.Minute)
	_, err = database.WriteDB().ExecContext(ctx, `
		UPDATE forge_archive_repos
		SET maintenance_succeeded_at = ?
		WHERE repo_id = ?`, maintenanceSucceededAt, newEntry.Repository.ID)
	require.NoError(err)

	require.NoError(database.QueueArchivePromptByIdentity(ctx, RepoIdentity{
		Platform: "github", PlatformHost: "github.com",
		Owner: "acme", Name: "widget",
	}, now.Add(3*time.Minute)))

	states, err := database.ListArchiveRepoStates(
		ctx, []int64{oldEntry.Repository.ID, newEntry.Repository.ID},
	)
	require.NoError(err)
	require.Len(states, 2)
	stateByRepoID := map[int64]ArchiveRepoState{}
	for _, state := range states {
		stateByRepoID[state.RepoID] = state
	}
	assert.Equal(
		ArchiveOperatorStatePaused,
		stateByRepoID[oldEntry.Repository.ID].OperatorState,
	)
	assert.Nil(stateByRepoID[newEntry.Repository.ID].MaintenanceSucceededAt)
}
...
func TestArchiveInventoryCompletedScanRejectsStaleDeliveries(t *testing.T) {
	...
	itemUpdatedAt := func(number int) time.Time {
		...
		require.NoError(d.ReadDB().QueryRowContext(ctx, `
			SELECT provider_updated_at FROM forge_archive_items
...
func TestArchiveStartFullPromotesDiscoveryAndPreservesResumedProgress(t *testing.T) {
	...
	require.NoError(err)
	_, err = d.WriteDB().ExecContext(ctx,
		`UPDATE forge_archive_dataset_progress SET status = 'complete' WHERE repo_id = ?`, repoID)
	require.NoError(err)
...
func TestArchiveClaimItemUsesEligibleDueWorkAndStableOrder(t *testing.T) {
	...
	_, err = d.WriteDB().ExecContext(ctx, `
		UPDATE forge_archive_items SET lifecycle_state = 'removed_upstream'
	...
	_, err = d.WriteDB().ExecContext(ctx, `
		UPDATE forge_archive_repos SET next_retry_at = ? WHERE repo_id = ?`,
...
func TestRequeueArchiveLifecycleDetailsOnlyReopensIncompleteGitHubRows(t *testing.T) {
	...
	require.NoError(database.EnsureDiscoveryArchives(ctx, []int64{repoID}, now))
	require.NoError(database.StartFullArchives(ctx, []int64{repoID}, now))
	prepareLifecycleArchiveCoverage(t, database, repoID, now)
...
func TestRequeueArchiveLifecycleDetailsIncludesSupportedProviderRows(t *testing.T) {
	t.Parallel()
...
func prepareLifecycleArchiveCoverage(
	...
	t.Helper()
	require := require.New(t)
	for _, itemType := range []ArchiveItemType{
		ArchiveItemTypeIssue, ArchiveItemTypeMergeRequest,
	} {
		require.NoError(database.CommitArchiveInventoryPage(t.Context(), ArchiveInventoryCommit{
			RepoID: repoID, ItemType: itemType,
			RefreshReason: ArchiveRefreshReasonInitial, ScanGeneration: 1,
			Exhausted: true, Now: now,
		}))
	}
	require.NoError(database.ReconcileArchiveCoverage(t.Context(), repoID, ArchiveCoverageSet{
		Issues: ArchiveCoverageSupported, MergeRequests: ArchiveCoverageSupported,
		Comments: ArchiveCoverageSupported, Reviews: ArchiveCoverageSupported,
		InlineComments: ArchiveCoverageSupported,
	}, now))
}
...
func TestArchiveGetProgressDerivesCountsFromDurableRows(t *testing.T) {
	...
	_, err := d.WriteDB().ExecContext(ctx, `
		UPDATE forge_archive_repos SET initial_completed_at = ?`, now)
	require.NoError(err)
	_, err = d.WriteDB().ExecContext(ctx, `
		UPDATE forge_archive_repo_scans SET status = 'complete'
	...
	_, err = d.WriteDB().ExecContext(ctx, `
		UPDATE forge_archive_dataset_progress SET next_retry_at = ?
...
func TestScanScopedRepositoryFailureIsClaimFenced(t *testing.T) {
	...
	_, err = d.WriteDB().ExecContext(ctx, `
		UPDATE forge_archive_repo_scans SET status = 'running'
		WHERE repo_id = ? AND scan = 'issue_inventory'`, repoID)
	require.NoError(err)
	applied, err = d.RecordArchiveRepositoryFailureForScan(
		ctx, repoID, ArchiveScanIssueInventory, 6,
