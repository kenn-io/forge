...
func TestQueuedCommentRefreshCannotRestoreCommentDeletedDuringFetch(t *testing.T) {
	t.Run("pull request", func(t *testing.T) {
		...
		mr := &db.MergeRequest{RepoID: repoID, PlatformID: 101, Number: 7, URL: "https://github.com/owner/repo/pull/7", Title: "PR", Author: "alice", State: "open", CreatedAt: now, UpdatedAt: now, LastActivityAt: now, CommentCount: 1, DetailFetchedAt: &now}
		mr.ID, err = database.UpsertMergeRequest(t.Context(), mr)
		require.NoError(err)
		commentID, body := int64(9001), "stale"
		comment := &gh.IssueComment{ID: &commentID, Body: &body, CreatedAt: &gh.Timestamp{Time: now}}
		require.NoError(database.UpsertMREvents(t.Context(), []db.MREvent{NormalizeCommentEvent(mr.ID, comment)}))

		fetchStarted := make(chan struct{})
		releaseFetch := make(chan struct{})
		client := &mockClient{listIssueCommentsIfChangedFn: func(context.Context, string, string, int) ([]*gh.IssueComment, error) {
			close(fetchStarted)
			<-releaseFetch
			return []*gh.IssueComment{comment}, nil
		}}
		repo := RepoRef{Platform: platform.KindGitHub, PlatformHost: "github.com", Owner: "owner", Name: "repo", RepoPath: "owner/repo"}
		syncer := NewSyncer(map[string]Client{"github.com": client}, database, nil, []RepoRef{repo}, time.Minute, nil, nil)

		refreshDone := make(chan struct{})
		go func() {
			syncer.refreshPRCommentsForItem(t.Context(), client, repo, mr)
			close(refreshDone)
		}()
		<-fetchStarted
		...
		go func() {
			_ = syncer.WithItemWriteLock(platform.KindGitHub, "github.com", "owner/repo", "pr", 7, func() error {
				close(deleteEntered)
				<-allowDelete
				return database.DeleteMRCommentEvent(t.Context(), mr.ID, commentID)
			})
			close(deleteDone)
		}()
		select {
	...
	t.Run("issue", func(t *testing.T) {
		...
		require.NoError(err)
		now := time.Now().UTC().Truncate(time.Second)
		issue := &db.Issue{RepoID: repoID, PlatformID: 201, Number: 8, URL: "https://github.com/owner/repo/issues/8", Title: "Issue", Author: "alice", State: "open", CreatedAt: now, UpdatedAt: now, LastActivityAt: now, CommentCount: 1, DetailFetchedAt: &now}
		issue.ID, err = database.UpsertIssue(t.Context(), issue)
		require.NoError(err)
		commentID, body := int64(9002), "stale"
		comment := &gh.IssueComment{ID: &commentID, Body: &body, CreatedAt: &gh.Timestamp{Time: now}}
		require.NoError(database.UpsertIssueEvents(t.Context(), []db.IssueEvent{NormalizeIssueCommentEvent(issue.ID, comment)}))
		...
		refreshDone := make(chan struct{})
		go func() {
			syncer.refreshIssueCommentsForItem(t.Context(), client, repo, issue)
			close(refreshDone)
		}()
		<-fetchStarted
		deleteEntered := make(chan struct{})
		...
		deleteDone := make(chan struct{})
		go func() {
			_ = syncer.WithItemWriteLock(platform.KindGitHub, "github.com", "owner/repo", "issue", 8, func() error {
				close(deleteEntered)
				<-allowDelete
				return database.DeleteIssueCommentEvent(t.Context(), issue.ID, commentID)
			})
			close(deleteDone)
		}()
		select {
...
func TestBulkCommentSyncRefetchesSnapshotsContainingDeletionReceipts(t *testing.T) {
	t.Run("pull request", func(t *testing.T) {
		require := require.New(t)
		database := openTestDB(t)
		repoID, err := database.UpsertRepo(t.Context(), db.GitHubRepoIdentity("github.com", "owner", "repo"))
		...
		require.NoError(err)
		mrID, err := database.UpsertMergeRequest(t.Context(), normalized)
		require.NoError(err)
		commentID, body := int64(9101), "stale bulk comment"
		comment := &gh.IssueComment{ID: &commentID, Body: &body, CreatedAt: &gh.Timestamp{Time: now}}
		require.NoError(database.UpsertMREvents(t.Context(), []db.MREvent{NormalizeCommentEvent(mrID, comment)}))
		require.NoError(database.RecordCommentDeletionAttempt(t.Context(), repoID, "pr", 7, commentID))
		require.NoError(database.DeleteMRCommentEvent(t.Context(), mrID, commentID))

		client := &mockClient{comments: []*gh.IssueComment{}}
		repo := RepoRef{Platform: platform.KindGitHub, PlatformHost: "github.com", Owner: "owner", Name: "repo", RepoPath: "owner/repo"}
		syncer := NewSyncer(map[string]Client{"github.com": client}, database, nil, []RepoRef{repo}, time.Minute, nil, nil)
		err = syncer.syncOpenMRFromBulk(t.Context(), repo, repoID, &BulkPR{PR: pr, Comments: []*gh.IssueComment{comment}, CommentsComplete: true}, false)
		require.NoError(err)

		events, err := database.ListMREvents(t.Context(), mrID)
		require.NoError(err)
		require.Empty(events)
		require.Equal(int32(1), client.listIssueCommentsCalled.Load())
	})
	...
	t.Run("issue", func(t *testing.T) {
		...
		require.NoError(err)
		now := time.Now().UTC().Truncate(time.Second)
		number, platformID, title, state, url := 8, int64(208), "Issue", "open", "https://github.com/owner/repo/issues/8"
		issue := &gh.Issue{ID: &platformID, Number: &number, Title: &title, State: &state, HTMLURL: &url, CreatedAt: &gh.Timestamp{Time: now}, UpdatedAt: &gh.Timestamp{Time: now}}
		normalized, err := NormalizeIssue(repoID, issue)
		require.NoError(err)
		issueID, err := database.UpsertIssue(t.Context(), normalized)
		require.NoError(err)
		commentID, body := int64(9102), "stale bulk comment"
		comment := &gh.IssueComment{ID: &commentID, Body: &body, CreatedAt: &gh.Timestamp{Time: now}}
		require.NoError(database.UpsertIssueEvents(t.Context(), []db.IssueEvent{NormalizeIssueCommentEvent(issueID, comment)}))
		require.NoError(database.RecordCommentDeletionAttempt(t.Context(), repoID, "issue", number, commentID))
		require.NoError(database.DeleteIssueCommentEvent(t.Context(), issueID, commentID))

		client := &mockClient{comments: []*gh.IssueComment{}}
		repo := RepoRef{Platform: platform.KindGitHub, PlatformHost: "github.com", Owner: "owner", Name: "repo", RepoPath: "owner/repo"}
		syncer := NewSyncer(map[string]Client{"github.com": client}, database, nil, []RepoRef{repo}, time.Minute, nil, nil)
		err = syncer.syncOpenIssueFromBulk(t.Context(), repo, repoID, &BulkIssue{Issue: issue, Comments: []*gh.IssueComment{comment}, CommentsComplete: true, TimelineComplete: true})
		require.NoError(err)
