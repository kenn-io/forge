...
type KataWorkspaceTargetResponse struct {
	...
	ResolutionStatus  string                     `json:"resolution_status,omitempty" enum:"mapped,unmapped,ambiguous,invalid,error"`
...
func (h *Handler) kataWorkspaceTargetForMetadata(
	...
	existingTarget, err := h.kataExistingWorkspaceTargetForIssue(ctx, metadata.DaemonID, metadata.IssueUID)
	if err != nil {
	...
	if err != nil {
		return unavailableKataWorkspaceTarget("error", ""), err
	}
	if resolution.Status != "mapped" {
		return unavailableKataWorkspaceTarget(resolution.Status, resolution.Source), nil
	}
...
func (h *Handler) kataExistingWorkspaceTargetForIssue(
	ctx context.Context,
	daemonID, issueUID string,
) (*KataWorkspaceTargetResponse, error) {
	existing, err := h.db.GetKataWorkspaceByIssue(ctx, daemonID, issueUID)
	if err != nil || existing == nil {
		return nil, err
	}
	target := h.kataExistingWorkspaceTarget(existing)
	return &target, nil
}

func unavailableKataWorkspaceTarget(status, source string) KataWorkspaceTargetResponse {
	reason := "Workspace creation is unavailable. Check the Kata project mapping in Settings."
	switch status {
	case "unmapped":
		reason = "No repository mapping matches this Kata project. Configure a mapping in Settings."
	case "ambiguous":
		reason = "Multiple repositories match this Kata project. Configure an explicit mapping in Settings."
	case "invalid":
		reason = "The configured Kata project mapping is invalid. Update it in Settings."
	case "error":
		reason = "Forge could not resolve the workspace repository. Retry, then check the server logs if it continues."
	}
	return KataWorkspaceTargetResponse{
		Available: false, ResolutionStatus: status, ResolutionSource: source, UnavailableReason: reason,
	}
}

func (h *Handler) kataExistingWorkspaceTarget(existing *db.Workspace) KataWorkspaceTargetResponse {
	repoRef := h.repoRefFromParts(
		existing.Platform, existing.PlatformHost, existing.RepoOwner, existing.RepoName,
	)
	return KataWorkspaceTargetResponse{
		Available:        true,
		ResolutionStatus: "mapped",
		ResolutionSource: "existing_workspace",
		Repo:             &repoRef,
		ItemType:         db.WorkspaceItemTypeKataTask,
		ItemKey:          existing.ItemKey,
		ExistingWorkspace: &workspaceapi.WorkspaceRef{
...
func (h *Handler) createKataWorkspace(
	...
	h.kataWorkspaceCreateMu.Lock()
	defer h.kataWorkspaceCreateMu.Unlock()
	existing, err := h.db.GetKataWorkspaceByIssue(ctx, metadata.DaemonID, metadata.IssueUID)
	if err != nil {
	...
	metadata, err = h.canonicalKataWorkspaceMetadata(ctx, metadata)
	if err != nil {
		return nil, err
	}
	resolution, err := h.resolveKataWorkspaceRepoResolution(ctx, metadata)
...
func (h *Handler) canonicalKataWorkspaceMetadata(
	ctx context.Context,
	...
	readCtx, cancel := context.WithTimeout(ctx, kataDaemonReadTimeout)
	defer cancel()
	reference, found, err := client.IssueReferenceByUID(readCtx, requested.IssueUID)
	if err != nil {
		return db.WorkspaceKataMetadata{}, kataUpstreamProblem(
			"verify Kata issue failed", requested.DaemonID,
		)
	}
	if !found {
		return db.WorkspaceKataMetadata{}, httpapi.NotFound(
			httpapi.CodeNotFound,
			"Kata issue not found",
			map[string]any{"daemon": requested.DaemonID, "issue_uid": requested.IssueUID},
		)
	}
	return db.WorkspaceKataMetadata{
		DaemonID:    strings.TrimSpace(requested.DaemonID),
		ProjectUID:  strings.TrimSpace(reference.ProjectUID),
		ProjectName: strings.TrimSpace(reference.ProjectName),
		IssueUID:    strings.TrimSpace(reference.UID),
		ShortID:     strings.TrimSpace(reference.ShortID),
		QualifiedID: strings.TrimSpace(reference.QualifiedID),
