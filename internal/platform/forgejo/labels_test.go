...
func forgejoLabelTestRef() platform.RepoRef {
...
func TestClientListLabelsFetchesRepoLabelCatalog(t *testing.T) {
	...
	client, err := NewClient("forgejo.test", testTokenSource("forgejo-token"), WithBaseURLForTesting(server.URL))
	require.NoError(err)

	catalog, err := client.ListLabels(context.Background(), forgejoLabelTestRef())
	require.NoError(err)
	...
	tests := []struct {
	...
	}{
		{
			...
			set: func(client *Client, ctx context.Context) ([]platform.Label, error) {
				return client.SetMergeRequestLabels(ctx, forgejoLabelTestRef(), 7, []string{"triage"})
			},
		},
		{
			name: "issue",
			set: func(client *Client, ctx context.Context) ([]platform.Label, error) {
				return client.SetIssueLabels(ctx, forgejoLabelTestRef(), 7, []string{"triage"})
			},
		},
	}
