...
- Phone-like PR detail routes have no inline workspace controller; creating or opening a workspace from them must land in the `/m/workspaces` shell, never the desktop `/terminal/{id}` route. The same phone signal renders the PR primary actions as one kit `AdaptiveActionGrid` (row or equal touch-sized tracks) instead of the desktop fit stages and actions menu. Desktop-narrow focus presentation keeps desktop destinations and layout (`frontend/src/App.svelte::phoneDetailProps`, `tests/e2e/mobile-pr-actions.spec.ts`).
