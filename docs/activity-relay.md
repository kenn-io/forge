...
Each Forge checks the relay every 30 seconds, then fetches the changed items
from GitHub using its own credentials.
...
The 30-second interval controls how often Forge checks the relay; GitHub
...
Forge after changing these settings. The default interval is `30s`; `60s`
also works. Remove the URL or set it to an empty string to disable the relay.
