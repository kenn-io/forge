...
func (s *Handlers) SwapGitHubNativeStackPreferenceLocked(enabled bool) bool {
	if (*s.Syncer) == nil {
		return false
	}
...
func (s *Handlers) ReconcileGitHubNativeStackProjection(previous, enabled bool) {
...
// for the whole sweep.
func (s *Handlers) RestoreBranchDerivedStackProjections() {
	if (*s.Syncer) == nil {
		return
	}
	ctx := s.BgCtx
...
func (s *Handlers) nativeStackProjectionRepoIDs(ctx context.Context) []int64 {
