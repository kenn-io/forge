...
- Issue comments, submitted reviews, and review threads have one canonical page implementation. Regular detail sync drains those page methods while archive hydration commits one page at a time; do not restore the deleted full-list GraphQL review-thread paginator or archive-only REST comment/review methods. (`internal/github/client.go::ListIssueComments`, `internal/github/client.go::ListReviews`, `internal/github/client.go::ListPullRequestReviewThreads`, `internal/github/archive_client.go::ListIssueCommentsPage`, `internal/github/archive_client.go::ListReviewsPage`, `internal/github/archive_client.go::ListArchiveReviewThreadsPage`)
- Archive requests use the shared sync budget but can spend only quadratic time-released surplus above `archiveLiveFloor`; the budget transport marks archive contexts separately so actual paginated calls, not admission estimates, drive archive spend. (`internal/github/budget.go`, `internal/github/budget_transport.go`, `internal/github/sync.go::Admit`)
