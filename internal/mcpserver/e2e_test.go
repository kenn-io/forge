...
import (
	...
	middlemanserver "go.kenn.io/middleman/internal/server"
...
func TestMCPToolsRoundTripAgainstDaemonAPI(t *testing.T) {
	...
	apiServer := middlemanserver.New(database, syncer, nil, "/", nil, middlemanserver.ServerOptions{
		APIAuthToken: token,
	...
	repos := callMCPTool[listReposOutput](t, session, "middleman_list_repos", map[string]any{})
	...
	search := callMCPTool[searchItemsOutput](t, session, "middleman_search_items", map[string]any{
		"query":      "Cache",
	...
	candidates := callMCPTool[findCandidatesOutput](t, session, "middleman_find_review_candidates", map[string]any{
		"since":      "2h",
	...
	contextOut := callMCPTool[getItemContextOutput](t, session, "middleman_get_item_context", map[string]any{
		"item":          item,
		"event_limit":   1,
		"include_stack": true,
	})
	...
	claim := callMCPTool[setWorkflowOutput](t, session, "middleman_set_item_workflow_state", map[string]any{
		"item":            item,
		"status":          "reviewing",
		"expected_status": "new",
		"actor":           "mcp-e2e",
...
func seedMCPPR(t *testing.T, database *db.DB, number int, title string) (int64, int64) {
	t.Helper()
	ctx := t.Context()
	repoID, err := database.UpsertRepo(ctx, db.GitHubRepoIdentity("github.com", "acme", "widgets"))
	require.NoError(t, err)
