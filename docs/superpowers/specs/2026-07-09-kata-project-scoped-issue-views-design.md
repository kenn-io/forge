...
Issue-view and search responses carry their concrete `daemon_id`. Kata workspace has a dedicated task client and binds an accepted result only after its request-generation guard passes, so stale results and palette/Docs/Messages operations cannot replace or inherit workspace provenance. Workspace navigation, automatic refreshes, task mutations, workspace creation identity, events, and streams stay pinned to that accepted daemon despite roster/default changes. An explicit switch clears every old daemon-scoped project, row, detail, cache, cursor, and binding before changing the active selector; replacement metadata and rows commit only after their paired loads succeed. A failed replacement is cleared before the previous daemon is bootstrapped again. If that rollback also fails, the selector points back to the previous daemon while daemon-scoped state stays empty, the stream stays stopped, and the connection error remains visible. The switching guard ignores overlapping switch requests. If a bound daemon is removed or becomes unhealthy, ordinary operations fail against it without falling through. Multi-request issue creation prefers the current workflow daemon and pins its optional metadata follow-up to the same concrete daemon.
