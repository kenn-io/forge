...
func (p *Provider) Capabilities() platform.Capabilities {
	_, workflows := p.client.(WorkflowCatalogAPI)
	_, workflowRuns := p.client.(WorkflowRunAPI)
	_, workflowDispatch := p.client.(WorkflowDispatchAPI)
...
type WorkflowCatalogAPI interface {
...
type WorkflowDispatchAPI interface {
...
func (p *Provider) ListManualWorkflows(
	...
	for _, workflow := range workflows {
		...
		definition, manual, parseErr := ParseManualWorkflow(
...
func (p *Provider) ListWorkflowEnvironments(
	ctx context.Context, ref platform.RepoRef,
...
func (p *Provider) GetWorkflowRun(ctx context.Context, ref platform.RepoRef, rawRunID string) (platform.WorkflowRun, error) {
	client, ok := p.client.(WorkflowRunAPI)
	if !ok {
...
func (p *Provider) ListWorkflowRunJobs(
