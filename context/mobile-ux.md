...
- Remember the selected session per workspace. A ready local workspace always offers its base terminal; when a runtime session exits, fall back to another live session or the base terminal.
...
- Routes retain local or Fleet identity. Authoritative provider deletion events and `workspaceNotFound` errors invalidate shared workspace state and replace matching phone routes with the list; unavailable Fleet hosts and generic connection failures remain in context with Retry or reconnect affordances (`frontend/src/lib/stores/workspace-host.svelte.ts::notifyWorkspaceDeleted`).
