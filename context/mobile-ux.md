...
- Phone terminals keep direct xterm interaction available for hardware keyboards and terminal-native controls. An optional bottom tray opens by tap or upward pull into an auto-growing composer with an explicit Send action; it stays above the software keyboard, feeds text through xterm's ordinary input event path, appends Enter, and clears only after xterm accepts the input (`frontend/src/lib/components/mobile/MobileWorkspaceTerminal.svelte::sendComposedInput`).
- Use xterm's canvas renderer on Android and Firefox; Android WebGL can accept terminal output while presenting a blank canvas (`frontend/src/lib/components/terminal/XtermTerminalPane.svelte::requiresCanvasRenderer`).
...
- A visible linked PR or issue action opens a workspace-scoped focused item view; Back restores the exact workspace and terminal selection, while browser Back from the terminal returns to the workspace list. The item view keeps those sessions claimed even when retention is zero (`frontend/src/App.svelte`).
- Stopping a phone terminal is destructive and must require an explicit confirmation before the runtime mutation begins (`frontend/src/lib/components/mobile/MobileWorkspaceTerminal.svelte::stopSelectedSession`).
