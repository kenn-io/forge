...
- Phone detail headers turn the title actions (edit, star, provider link) into bordered, equal hit-target squares; the inline `#123` copy button stays text-sized and must not receive the hit-target minimums, or the meta row grows to button height (`frontend/src/App.svelte::.focus-layout--phone`).
