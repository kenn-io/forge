      ...
      const first = yield* workflow
        .mutate({
          ...
          mutation: Deferred.succeed(firstStarted, undefined).pipe(
            Effect.andThen(Ref.update(observed, (events) => [...events, "cancel"])),
    ...
    Effect.gen(function* () {
      ...
      const exit = yield* workflow
        .mutate({
          key: "job:1",
          operation: "cancel Roborev job",
          mutation: Ref.update(attempts, (count) => count + 1).pipe(Effect.andThen(Effect.fail("rejected"))),
          reconcile: () => Effect.succeed(Option.some(undefined)),
        })
        .pipe(Effect.exit);
...
it.effect("keeps an acknowledged mutation successful when its authority refresh fails", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const workflow = yield* makeRoborevWorkflow;
      ...
      const result = yield* workflow.mutate({
        ...
        onAcknowledgedRefreshFailure: () => Ref.update(refreshFailures, (count) => count + 1),
      });
...
it.effect("reconciles a retained uncertain mutation before accepting another write for its target", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const workflow = yield* makeRoborevWorkflow;
      ...
      const originalReconcile = Ref.get(authorityAvailable).pipe(
        Effect.flatMap((available) =>
          available ? Effect.succeed(Option.none<string>()) : Effect.fail(new Error("authority unavailable")),
        ),
      );
      ...
      const replacement = yield* workflow.mutate({
        key: "review:42",
        operation: "add Roborev comment",
        mutation: Ref.update(replacementAttempts, (count) => count + 1).pipe(Effect.as("replacement")),
...
it.effect("distinguishes an original uncertain mutation from a later blocked action", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const workflow = yield* makeRoborevWorkflow;
      ...
      if (original._tag === "Failure" && blocked._tag === "Failure") {
        assert.include(String(original.cause), "RoborevMutationOutcomeUnknown");
        assert.include(String(blocked.cause), "RoborevMutationBlockedUnknown");
      }
    }),
  ),
);

it.effect("stops active and pending panel refreshes for only the released owner", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const workflow = yield* makeRoborevWorkflow;
      const firstStarted = yield* Deferred.make<void>();
      ...
      const panelOptions = (read: Effect.Effect<string>) => ({
        read,
        onStart: Effect.void,
        onSuccess: (value: string) => Ref.update(published, (values) => [...values, value]),
      ...
      yield* Deferred.await(firstStarted);
      yield* workflow.panel("released-owner", "panel-1", panelOptions(Effect.succeed("released-pending")));
      yield* workflow.panel(
        "other-owner",
        "panel-2",
        panelOptions(
