...
- A linked item opened from the workspace list returns to the list; one opened from a terminal restores that exact workspace and selected session. A direct item deep link falls back to its workspace terminal, and item views keep sessions claimed even when retention is zero (`frontend/src/App.svelte::leaveMobileWorkspaceItem`).
