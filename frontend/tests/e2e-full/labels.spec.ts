...
test.describe("label editing", () => {
  async function openLabelsFromPalette(
    ...
    await expect(page.getByRole("button", { name: /Edit labels/ })).toHaveCount(
      1,
    );
    await page.getByRole("button", { name: /Edit labels/ }).click();
    await expect(
      page.getByRole("dialog", { name: "Edit labels" }),
    ).toBeVisible();
  }
    ...
    try {
      ...
      const updateResponse = page.waitForResponse(
        (response) =>
          ...
          response.url() ===
            `${baseURL}/api/v1/pulls/github/acme/widgets/1/labels`,
      );
  ...
  test("keeps label editing in the header when no labels are assigned", async ({
    ...
    try {
      ...
      await expect(page.locator(".pull-detail .label-editor-row")).toHaveCount(
        0,
      );
      await expect(
        page
      ...
      await expect(
        page.locator(".pull-detail .label-editor-empty"),
      ).toHaveCount(0);
    } finally {
      await isolatedServer?.stop();
    }
    ...
    try {
      ...
      const updateResponse = page.waitForResponse(
        (response) =>
          response.request().method() === "PUT" &&
          response.url() ===
            `${baseURL}/api/v1/issues/github/acme/widgets/10/labels`,
      );
      await page.getByRole("button", { name: "Clear selected labels" }).click();
      expect((await updateResponse).status()).toBe(200);
