...
export class Microtasks extends Context.Service<Microtasks, MicrotaskFactory>()("kenn-forge/browser/Microtasks") {}
...
export function makeMicrotaskScheduler<R>(
  ...
  return Effect.gen(function* () {
    const runMicrotask = yield* FiberHandle.makeRuntime<Microtasks | R, never, void>();
    let generation = 0;
    let scheduled = false;
    ...
    return {
      ...
      schedule: () => {
        ...
        activeMicrotask = runMicrotask(
          nextMicrotask.pipe(
            Effect.andThen(Effect.suspend(() => (requestGeneration === generation ? onMicrotask : Effect.void))),
            Effect.ensuring(
              Effect.sync(() => {
