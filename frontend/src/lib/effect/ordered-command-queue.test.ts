...
import { makeOrderedCommandQueue } from "./ordered-command-queue.js";
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const queue = yield* makeOrderedCommandQueue(
        ...
        (command: number) => Ref.update(observed, (commands) => [...commands, command]),
        2,
      );

      const submissions = yield* Effect.all([queue.submit(1), queue.submit(2), queue.submit(3)], {
...
it.effect("acknowledges one command failure without stopping later commands", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const observed = yield* Ref.make<ReadonlyArray<number>>([]);
      const queue = yield* makeOrderedCommandQueue(
        ...
        (command: number) =>
          Ref.update(observed, (commands) => [...commands, command]).pipe(
            Effect.andThen(command === 2 ? Effect.fail("rejected") : Effect.succeed(command)),
          ),
        2,
      );

      const exits = yield* Effect.all([queue.submit(1), queue.submit(2), queue.submit(3)].map(Effect.exit), {
        concurrency: "unbounded",
      });
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const third = yield* Effect.forkChild(
        queue.submit(3).pipe(Effect.andThen(Deferred.succeed(thirdCompleted, undefined))),
      );
      yield* Effect.yieldNow;
...
it.effect("completes active and pending acknowledgements when shut down", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const queue = yield* makeOrderedCommandQueue(
        "shutdown commands",
        () => Deferred.succeed(activeStarted, undefined).pipe(Effect.andThen(Deferred.await(active))),
        1,
      );
      const first = yield* Effect.forkChild(Effect.exit(queue.submit("first")));
      yield* Deferred.await(activeStarted);
      const second = yield* Effect.forkChild(Effect.exit(queue.submit("second")));
      yield* Effect.yieldNow;
