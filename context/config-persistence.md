...
- `[service]` is opt-in and startup-bound; reject policy changes during reload
  before replacing credential sources, so a live service cannot acquire personal
  credential fallbacks (`internal/server/config_reload.go::applyConfigChange`).
