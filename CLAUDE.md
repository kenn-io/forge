...
**Mocking the API is a second, independent decision from the lane — and the default is "don't."** Prefer the real Go backend; get as much through it as the behavior allows. The fetch/`page.route` mock is a hand-maintained mirror of the Go wire shape, so a mock-only test can pass against a contract the backend no longer honors. Use the rule:

- Behavior whose correctness **depends on backend logic** — sync, persistence, capability computation, provider normalization, error envelopes, or the exact response wire shape — must be exercised against the **real Go backend** (`tests/e2e-full/` with the e2e-server, or Go API tests in `internal/server/`). Do not assert backend-computed values (a seeded `budget_spent`, a synced state) through a hand-written mock; that only re-asserts the fixture.
- Mock the API only for **pure-frontend behavior** that is indifferent to where the data came from — keyboard, focus, rendering permutations, routing, request-shape checks. When a feature has both, split it: real-backend coverage for the contract, the fast mock lane for the interaction matrix.
- When you do mock, change fixtures and route matching in `src/test/mockApiFetch.ts` only (never fork the Playwright copy), and keep the shapes faithful to what Go actually emits.
