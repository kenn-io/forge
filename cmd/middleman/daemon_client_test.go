...
func TestDaemonOriginTransportScopesHeadersToDaemonOrigin(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)
	var redirectedAuthorization string
	var redirectedForwardedHost string
	target := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		redirectedAuthorization = r.Header.Get("Authorization")
		redirectedForwardedHost = r.Header.Get("X-Forwarded-Host")
		w.WriteHeader(http.StatusNoContent)
	}))
	t.Cleanup(target.Close)
	var originAuthorization string
	var originForwardedHost string
	origin := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		originAuthorization = r.Header.Get("Authorization")
		originForwardedHost = r.Header.Get("X-Forwarded-Host")
