...
- Capture the monitored-repository scope as stable internal IDs under the repository-reconciliation fence, then apply it inside Activity and actor-candidate SQL before ordering and limits; mutable routes or handler-side filtering can hide valid rows (`internal/server/activityapi/huma_routes.go::Handlers.trackedActivityRepoIDsUnderRepositoryReconciliationRead`).
