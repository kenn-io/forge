...
async function openPRTimeline(page: Page): Promise<void> {
  ...
  await expect(page.locator(".pull-detail .detail-title")).toContainText("Add widget caching layer");
}
...
test.describe("item references through the full stack", () => {
  test("routes tracked cross-repository timeline references internally", async ({ page }) => {
    ...
    await expect.poll(() => new URL(page.url()).pathname).toBe("/pulls/github/acme/tools/1");
    await expect((await response).ok()).toBe(true);
    await expect(page.locator(".pull-detail .detail-title")).toContainText("Add CLI flag parser");
  });
  ...
  test("opens provider fallback URLs for untracked timeline references", async ({ page, context }) => {
    ...
    await expect((await response).ok()).toBe(true);
    await expect.poll(() => new URL(page.url()).pathname).toBe("/pulls/github/acme/widgets/1");
  });
});
