...
interface CommandEntry<Input, Output, Error> {
  ...
  readonly acknowledgement: Deferred.Deferred<Exit.Exit<Output, Error | CommandQueueClosed>>;
}
...
export const makeOrderedCommandQueue = <Input, Output, Error, Requirements>(
  ...
  capacity = 64,
): Effect.Effect<OrderedCommandQueue<Input, Output, Error>, never, Requirements | Scope> =>
  Effect.gen(function* () {
    const scope = yield* Effect.scope;
    const commands = yield* Queue.bounded<CommandEntry<Input, Output, Error>>(capacity);
    const closed = yield* Ref.make(false);
    const pending = new Set<Deferred.Deferred<Exit.Exit<Output, Error | CommandQueueClosed>>>();
    ...
    const shutdown = Effect.gen(function* () {
      ...
      yield* Effect.forEach(acknowledgements, (acknowledgement) => Deferred.succeed(acknowledgement, exit), {
        discard: true,
      });
