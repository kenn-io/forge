...
- Archive calls return one bounded page with an advancing opaque cursor or explicit exhaustion. Archive page commits write normalized rows and cursor progress atomically; ordinary comments reconcile at exhaustion, while reviews and threads remain additive. Authoritative normal-sync children may satisfy only the dataset families fetched completely for the current parent revision. (`internal/platform/types.go::ValidatePage`, `internal/db/queries_dataset_progress.go::CommitDatasetPage`, `internal/db/queries_snapshot_children.go::satisfyArchiveDatasetsFromNormalSyncTx`)
- Missing-child reconciliation must not consume one SQLite bind variable per retained record. (`internal/db/queries.go::deleteMissingIssueCommentEventsTx`, `internal/db/queries_review.go::deleteMissingMRReviewThreadsTx`)
