...
describe("WorkspaceListSidebar", () => {
  ...
  it("shows fork PR drift against the PR head without offering branch sync", async () => {
    mockGet.mockResolvedValue({
      data: {
        workspaces: [
          {
    ...
    expect(container.querySelector(".push-state")?.getAttribute("title")).toBe("0 ahead, 12 behind PR head");
    await fireEvent.contextMenu(container.querySelector(".ws-row")!);
