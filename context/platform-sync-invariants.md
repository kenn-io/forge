...
- Archive admission is provider-host scoped and request-bounded. Normal index, notification, and active-detail work outrank archive requests; one archive lease covers only one upstream page and is released before SQLite staging or publication. Register repo index and item detail work inside their shared execution functions so periodic, watched, manual, API, and item-number entry points cannot bypass admission. (`internal/github/sync.go::beginProviderWork`, `internal/github/sync.go::tryBeginArchiveProviderRequest`, `internal/github/sync.go::Admit`, `internal/archive/scheduler.go::admit`)
- Archive spend is counted separately inside the shared hourly budget. Preserve a provider-specific hard live floor, release archive surplus quadratically toward a fresh observed reset, and disable archive requests when reset timing is unknown or the configured budget cannot cover the floor. The provider rate reserve remains an independent hard gate. (`internal/github/budget.go::CanSpendArchive`, `internal/github/budget.go::ArchiveSpendCeiling`, `internal/github/budget_transport.go::WithArchiveSyncBudget`, `internal/github/sync.go::archiveLiveFloor`)
- Live and archive child datasets use the same transaction helpers for issue comments, MR event families, and review threads. Live callers may update derived parent fields; archive publication preserves current provider-detail metadata while retaining archive snapshot and staged-page preconditions. (`internal/db/queries_snapshot_children.go::replaceIssueCommentsDatasetTx`, `internal/db/queries_snapshot_children.go::replaceMREventDatasetTx`, `internal/db/queries_snapshot_children.go::replaceMRReviewThreadsDatasetTx`)
