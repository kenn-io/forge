...
import { formatTokenCount, parseCostUsd, parseTokenUsage, tokenUsageDetail, tokenUsageStats } from "./roborev-usage.js";
...
describe("parseTokenUsage", () => {
  ...
  it("ignores cost when the blob reports no cost data", () => {
    const usage = parseTokenUsage(JSON.stringify({ total_output_tokens: 100, cost_usd: 5, has_cost: false }));
    expect(usage).toEqual({
      inputTokens: null,
...
describe("parseCostUsd", () => {
  it("extracts priced cost and stays null otherwise", () => {
    expect(parseCostUsd(JSON.stringify({ cost_usd: 0.42, has_cost: true }))).toBe(0.42);
    expect(parseCostUsd(JSON.stringify({ cost_usd: 0.42, has_cost: false }))).toBeNull();
    expect(parseCostUsd(JSON.stringify({ cost_usd: "0.42", has_cost: true }))).toBeNull();
...
describe("tokenUsageStats", () => {
  it("emits only the stats the blob reported", () => {
    const usage = parseTokenUsage(JSON.stringify({ total_output_tokens: 2542, peak_context_tokens: 47248 }));
