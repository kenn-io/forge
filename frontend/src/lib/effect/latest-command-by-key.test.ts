...
import { makeLatestCommandByKey } from "./latest-command-by-key.js";

it.effect("runs the active command and only the latest pending command per key", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const commands = yield* makeLatestCommandByKey<never>("latest commands");
      const firstStarted = yield* Deferred.make<void>();
      const releaseFirst = yield* Deferred.make<void>();
      const executed = yield* Ref.make<ReadonlyArray<string>>([]);
      const command = (value: string, wait: Effect.Effect<void> = Effect.void) =>
        Ref.update(executed, (values) => [...values, value]).pipe(Effect.andThen(wait));
      ...
      const superseded = yield* Effect.forkChild(commands.submit("pull", command("second")));
      const latest = yield* Effect.forkChild(commands.submit("pull", command("third")));
...
it.effect("continues with the latest pending command after an active failure", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const commands = yield* makeLatestCommandByKey<string>("latest commands");
      const firstStarted = yield* Deferred.make<void>();
      const releaseFirst = yield* Deferred.make<void>();
      const executed = yield* Ref.make<ReadonlyArray<string>>([]);
      const first = yield* Effect.forkChild(
...
it.effect("does not execute a pending command after a blocking failure", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const commands = yield* makeLatestCommandByKey<string>("latest commands", (failure) => failure === "review");
      const firstStarted = yield* Deferred.make<void>();
      const releaseFirst = yield* Deferred.make<void>();
      ...
      if (Exit.isFailure(firstExit) && Exit.isFailure(pendingExit)) {
        assert.deepStrictEqual(Cause.findErrorOption(firstExit.cause), Option.some("review"));
        assert.deepStrictEqual(Cause.findErrorOption(pendingExit.cause), Option.some("review"));
      }
...
it.effect("completes active and pending acknowledgements when it shuts down", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const commands = yield* makeLatestCommandByKey<never>("latest commands");
      const started = yield* Deferred.make<void>();
      const active = yield* Effect.forkChild(
        commands.submit("pull", Deferred.succeed(started, undefined).pipe(Effect.andThen(Effect.never))),
      );
      yield* Deferred.await(started);
      const pending = yield* Effect.forkChild(commands.submit("pull", Effect.void));
