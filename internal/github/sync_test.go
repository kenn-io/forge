...
func TestSyncNotificationsForRepoSkipsRemovedConfiguration(t *testing.T) {
	require := require.New(t)
	assert := assert.New(t)
	d := openTestDB(t)
	repo := RepoRef{
	...
	t.Cleanup(syncer.Stop)
	require.NoError(syncer.SetReposWithContext(t.Context(), nil, false))
...
func TestRepositoryReplacementPublishesIdentityForDetailAndNotifications(
	t *testing.T,
) {
	require := require.New(t)
	assert := assert.New(t)
	...
	client := &mockClient{
		getRepositoryFn: func(
			...
			return &gh.Repository{
				NodeID: new(newRepo.PlatformExternalID),
				Name:   new(newRepo.Name),
				Owner:  &gh.User{Login: new(newRepo.Owner)},
			}, nil
		},
		getPullRequestFn: func(
			context.Context, string, string, int,
		) (*gh.PullRequest, error) {
			return buildOpenPR(1, time.Now().UTC()), nil
		},
		listNotificationsFn: func(
			context.Context,
			NotificationListOptions,
	...
	assert.Equal(newRepo.PlatformExternalID, tracked[0].PlatformExternalID)
	...
	require.NoError(syncer.syncNotificationsForRepo(
		t.Context(),
		platform.KindGitHub,
		"github.com",
		client,
	...
	assert.Positive(notificationCalls.Load())
	repos, err := d.ListRepos(t.Context())
	require.NoError(err)
	require.Len(repos, 1)
...
func TestHoldRepoMutationIncarnationRejectsLoadedReplacement(t *testing.T) {
	require := require.New(t)
	d := openTestDB(t)
	oldRepo := RepoRef{
		Platform: platform.KindGitHub, PlatformHost: "github.com",
	...
	require.NoError(err)
	loaded, err := d.GetRepoByID(t.Context(), oldRepoID)
	require.NoError(err)
	...
	_, err = d.UpsertRepoByProviderID(
		t.Context(),
		platform.DBRepoIdentity(platformRepoRef(newRepo)),
	)
	require.NoError(err)

	_, release, err := syncer.HoldRepoMutationIncarnation(
		t.Context(),
		*loaded,
	)
	defer release()
...
func TestSyncRepoRejectsLiveRenameUntilConfigurationChanges(t *testing.T) {
	require := require.New(t)
	assert := assert.New(t)
	ctx := t.Context()
	d := openTestDB(t)
	...
	client := &mockClient{
		getRepositoryFn: func(
			...
			require.Contains([]string{"widgets", "renamed-widgets"}, name)
			return &gh.Repository{
	...
	for range 2 {
		err := syncer.syncRepo(ctx, configured)
		require.ErrorIs(err, ErrConfiguredRepoIdentityChanged)
	}
	assert.Zero(listCalls.Load())
	repos, err := d.ListRepos(ctx)
	require.NoError(err)
	assert.Empty(repos)
	...
	require.NoError(syncer.SetReposWithContext(ctx, []RepoRef{renamed}, false))
	require.NoError(syncer.syncRepo(ctx, renamed))
	assert.Positive(listCalls.Load())
}
...
func TestReconcileRepoIdentityRejectsRedirectIntoOccupiedPath(t *testing.T) {
	require := require.New(t)
	assert := assert.New(t)
	d := openTestDB(t)
	sourceRepo := RepoRef{
	...
	_, _, _, _, err = syncer.reconcileRepoIdentity(
		t.Context(),
		sourceRepo,
	)
	require.ErrorIs(err, ErrConfiguredRepoIdentityChanged)
	...
	require.NoError(err)
	require.Len(repos, 2)
	byID := map[int64]db.Repo{
		repos[0].ID: repos[0],
		repos[1].ID: repos[1],
	}
	assert.Equal(sourceRepo.PlatformExternalID, byID[sourceID].PlatformRepoID)
	assert.Equal(destinationRepo.PlatformExternalID, byID[destinationID].PlatformRepoID)
}
