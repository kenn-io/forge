...
- `modes.actions` is a false-by-default UI visibility preference: it gates every Actions surface while provider workflow capabilities and HTTP routes remain available (`internal/config/config.go::ModeVisibility`).
