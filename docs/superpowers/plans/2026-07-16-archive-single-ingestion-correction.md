...
2. When `Progress != nil`: CAS — `InputCursor == next_cursor` advances (`page_count++`, bound `maxScanPages = 10_000` per generation → exceed marks `blocked` with `last_error_code = "page_bound"` and returns `*ScanBlockedError`); `InputCursor == last_input_cursor` (same generation) is the idempotent replay → return `nil` writing nothing; anything else → `*StaleDatasetProgressError`. Only directly detected cursor errors (provider-invalidated or in-window echoed cursors) use `last_error_code = "invalid_cursor"`; reaching the bound uses the deliberately ambiguous `page_bound`, which may be an oversized scan or an undetected cycle — the Task 12 continue mode is an explicit operator classification, not an automatic one.
