...
```sh
KENN_FORGE_GITLAB_CONTAINER_E2E=1 make test-gitlab-container
```

The test starts `gitlab/gitlab-ce:19.1.1-ce.0` through testcontainers' Docker Compose module, waits for GitLab Rails to serve the sign-in page, runs `bootstrap.sh`, and syncs the seeded project into a real SQLite database. Override the image with `KENN_FORGE_GITLAB_IMAGE` when checking a different GitLab release.

By default the Go test uses the Compose project `kenn-forge-gitlab-e2e`, maps GitLab to a free loopback port, and runs `docker compose down` without `-v` during cleanup. The named GitLab volumes are kept so repeated local runs do not pay the full database initialization cost. Set `KENN_FORGE_GITLAB_COMPOSE_PROJECT` or `GITLAB_HTTP_PORT` if you need a specific project name or port.
...
GITLAB_URL=http://127.0.0.1:${GITLAB_HTTP_PORT:-18080} \
  ...
  scripts/e2e/gitlab/bootstrap.sh /tmp/kenn-forge-gitlab-manifest.json
...
Set `KENN_FORGE_KEEP_GITLAB_FIXTURE=1` only when you want to leave the test's Compose stack running after the Go test exits.
...
KENN_FORGE_GITLAB_IMAGE=kenn-forge/gitlab-ce-fixture:19.1.1-ce.0 \
  KENN_FORGE_GITLAB_CONTAINER_E2E=1 \
  make test-gitlab-container
```

The bake flow uses `docker-bake.hcl` to build a small runtime layer, starts it once, runs `bootstrap.sh`, exports GitLab's official volume paths into tarballs under `scripts/e2e/gitlab/fixture-data/`, and then builds `kenn-forge/gitlab-ce-fixture:<tag>` with those tarballs. The final image restores the tars into fresh runtime volumes before GitLab starts.
