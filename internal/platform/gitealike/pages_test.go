...
func TestOpenScansMatchLegacyLists(t *testing.T) {
	...
	require.NoError(err)
	legacyIssues, err := provider.ListOpenIssues(t.Context(), ref)
	require.NoError(err)
	legacyMRs, err := provider.ListOpenMergeRequests(t.Context(), ref)
	require.NoError(err)

	assert.True(issuePage.Exhausted)
	assert.True(mrPage.Exhausted)
	assert.Equal(legacyIssues, issuePage.Items)
	assert.Equal(legacyMRs, mrPage.Items)
