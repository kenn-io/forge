...
test.describe("focus mode", () => {
  test("PR focus route renders detail without shell chrome", async ({
    page,
  }) => {
  ...
  test("issue focus route renders detail without shell chrome", async ({
    page,
  }) => {
  ...
  test("narrow PR focus route shows actions only inside the actions menu", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 320, height: 720 });
    await page.goto("/focus/pulls/github/acme/widgets/1");
    await page
      .locator(".focus-layout .pull-detail")
      .waitFor({ state: "visible", timeout: 10_000 });
    ...
    await expect(
      page.locator(".actions-row--primary .btn--approve"),
    ).toBeHidden();
    await expect(
      page.locator(".actions-menu-popover .btn--approve"),
    ).toBeVisible();
    await expect(
      page.locator(".actions-menu-popover .btn--merge"),
    ).toBeVisible();
    await expect(
      page.locator(".actions-menu-popover .btn--close"),
    ).toBeVisible();
    await expect(
      page
        .locator(".actions-menu-popover")
        .getByRole("button", { name: "Labels" }),
    ).toBeVisible();
    await expect(
      page.locator(".actions-menu-popover .btn--workspace"),
    ).toBeVisible();
    ...
    await expect(page.locator(".label-editor-backdrop")).toHaveCSS(
      "background-color",
    ...
    if (pickerRect && viewport) {
      ...
      expect(pickerRect.y + pickerRect.height).toBeLessThanOrEqual(
        viewport.height - 20,
      );
      expect(
        Math.abs(pickerRect.x + pickerRect.width / 2 - viewport.width / 2),
  ...
  test("narrow PR focus route closes actions menu when Labels toggles an open picker", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 320, height: 720 });
    await page.goto("/focus/pulls/github/acme/widgets/1");
  ...
  test("PR focus route dismisses the inline label picker on outside click", async ({
    page,
  }) => {
  ...
  test("PR focus route keeps workspace inside the mid-narrow action grid", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator(
        ".actions-row--primary .primary-workspace-action .btn--workspace",
      ),
    ).toBeVisible();
    await expect(page.locator(".actions-row--workspace")).toBeHidden();
    ...
    await expect(
      page.locator(
        ".actions-row--primary .primary-workspace-action .action-error",
      ),
    ).toHaveText("workspace setup failed");
  });

  test("narrow merged PR focus route keeps labels and workspace actions available", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 320, height: 720 });
