...
export function parseTokenUsage(tokenUsage: string | undefined): JobTokenUsage | null {
...
export function tokenUsageStats(usage: JobTokenUsage): TokenUsageStat[] {
  ...
  if (usage.peakContextTokens !== null) {
    stats.push({ label: "peak", value: formatTokenCount(usage.peakContextTokens) });
  }
  return stats;
}
...
export function tokenUsageDetail(usage: JobTokenUsage): string {
  ...
  const parts: string[] = [];
  if (usage.inputTokens !== null) parts.push(`input ${group(usage.inputTokens)}`);
  if (usage.cachedInputTokens !== null) {
    parts.push(`cached input ${group(usage.cachedInputTokens)}`);
  }
  if (usage.outputTokens !== null) parts.push(`output ${group(usage.outputTokens)}`);
  if (usage.peakContextTokens !== null) {
