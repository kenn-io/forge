...
- Archive calls return one bounded page with an advancing opaque cursor or explicit exhaustion. Whole-dataset live readers collect the same canonical page methods rather than maintaining parallel endpoint or normalization stacks. Cumulative staging limits count payload and cursor bytes; equal-timestamp parent advances rebind unfinished pages instead of restarting them, while authoritative normal-sync children satisfy only the dataset families fetched completely. Page commits reject already-complete datasets, and publication/finalization still verify the live snapshot. (`internal/platform/types.go::ValidateArchivePage`, `internal/platform/page_collect.go::CollectArchivePages`, `internal/github/client.go::ListPullRequestReviewThreads`, `internal/db/queries_archive.go::reconcileArchiveItemSnapshotTx`, `internal/db/queries_snapshot_children.go::satisfyArchiveDatasetsFromNormalSyncTx`)
...
- Archive admission is provider-host scoped and request-bounded. Normal index, notification, and active-detail work outrank archive requests; live work registers first, cancels the active archive request context, and waits for that request lease to release before provider I/O. Archive leases are released before SQLite staging or publication. Register repo index and item detail work inside their shared execution functions so periodic, watched, manual, API, and item-number entry points cannot bypass admission. (`internal/github/sync.go::beginProviderWork`, `internal/github/sync.go::tryBeginArchiveProviderRequest`, `internal/github/sync.go::Admit`, `internal/archive/scheduler.go::admit`)
- Archive spend is counted separately per actual HTTP attempt inside the shared hourly budget, including authentication retries on every provider. Preserve a provider-specific hard live floor, reject declared costs that would cross the provider rate reserve, release archive surplus quadratically toward a fresh observed reset, and retry ramp denials within the current window rather than sleeping until reset. Unknown reset timing or a configured budget that cannot cover the floor disables archive requests. (`internal/github/budget.go::CanSpendArchive`, `internal/github/budget_transport.go::WrapSyncBudgetTransport`, `internal/github/sync.go::Admit`, `internal/github/sync.go::archiveLiveFloor`)
- Live and archive child datasets use the same transaction helpers for issue comments, MR event families, and review threads. Ordinary comments replace current provider truth; submitted reviews, inline review comments, and review threads are stable-identity additive and are never deleted merely because a later response omits them. Live callers may update derived parent fields; archive publication preserves current provider-detail metadata while retaining archive snapshot and staged-page preconditions. (`internal/db/queries_snapshot_children.go::replaceIssueCommentsDatasetTx`, `internal/db/queries_snapshot_children.go::replaceMREventDatasetTx`, `internal/db/queries_snapshot_children.go::replaceMRReviewThreadsDatasetTx`)
...
Window replay cost is provider-declared and bounded; multi-request replay exhaustion requires a page-one confirmation, while excess depth returns `page_limit` and durably blocks only that scan. (`internal/platform/types.go::ArchiveCapabilities`, `internal/archive/inventory.go::archiveInventoryPageCost`, `internal/platform/gitlab/pages.go::gitLabWindowedCursorPage`, `internal/archive/service.go::scanBlockCode`)
