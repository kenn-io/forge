...
func RenderMarkdown(model Model) (string, error) {
	...
	out.WriteString("\n## Contributors\n\n")
	out.WriteString("| Provider | Host | Login | Issues | Pull/Merge Requests | Comments | Reviews | Inline Comments | Total |\n")
	out.WriteString("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |\n")
	for _, contributor := range model.Contributors {
		...
		fmt.Fprintf(
			&out, "| %s | %s | %s | %d | %d | %d | %d | %d | %d |\n",
			tableCell(contributor.Provider), tableCell(contributor.PlatformHost), tableCell(login),
			contributor.Counts.IssuesOpened, contributor.Counts.MergeRequestsOpened,
