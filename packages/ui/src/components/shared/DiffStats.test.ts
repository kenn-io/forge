...
describe("DiffStats", () => {
  ...
  it("formats additions and deletions consistently", () => {
    ...
    expect(stats.querySelector(".diff-stats__add")?.textContent?.trim())
      .toBe("+2.78k");
    expect(stats.querySelector(".diff-stats__del")?.textContent?.trim())
      .toBe("−216");
  });
  ...
  it("dims zero values when requested", () => {
    ...
    expect(stats.querySelector(".diff-stats__add")?.classList
      .contains("diff-stats__value--dim")).toBe(true);
    expect(stats.querySelector(".diff-stats__del")?.classList
      .contains("diff-stats__value--dim")).toBe(false);
  });
});
