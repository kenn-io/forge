...
func TestWritePrivateKeyRejectsHostileSlugs(t *testing.T) {
	...
	_, err := writePrivateKey(configPath, "../up", "kenn-forge-ok", "pem")
	require.Error(err, "hostile host must be rejected")

	path, err := writePrivateKey(configPath, "github.com", "kenn-forge-3f9a2c", "pem-bytes")
	require.NoError(err)
	...
	other, err := writePrivateKey(configPath, "ghe.example.com", "kenn-forge-3f9a2c", "pem-2")
	require.NoError(err)
