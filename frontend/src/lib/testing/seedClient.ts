...
// run by ./seedServer.globalSetup.ts, which provides its base URL here. These
// helpers fetch real responses from it so tests assert against the exact
// shapes the app sees in production, instead of checked-in JSON that silently
// drifts from the server. The server is seeded deterministically, so value
