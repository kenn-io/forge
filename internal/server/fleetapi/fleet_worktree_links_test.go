...
// newLinkTriggerServer builds a minimal Handler with a real syncer (the
...
func newLinkTriggerServer(
	...
	hub := newTestEventHub()
	handler := New(Deps{DB: database, Syncer: syncer, Broadcast: hub.Broadcast})
	workspaceAPI := workspaceapi.New(workspaceapi.Deps{
		DB:                     database,
		Syncer:                 syncer,
		RecomputeWorktreeLinks: handler.recomputeWorktreeLinksNow,
	})
	return &linkTriggerServer{Handler: handler, workspaceAPI: workspaceAPI},
		func() int { return countLinkHints(hub) }
}

type linkTriggerServer struct {
	*Handler
...
func countLinkHints(hub *testEventHub) int {
	return hub.count("worktree_links_changed")
}
...
func TestNotifyWorktreeChangeBroadcasts(t *testing.T) {
	...
	hub := newTestEventHub()
	srv := New(Deps{Broadcast: hub.Broadcast})
