...
- `buildActivityFilterTypes` combines enabled item-opening, event, branch, and notification types for `/activity`; only the fully enabled state uses empty `[]`, which means no backend type filter (`packages/ui/src/stores/activity.svelte.ts::buildActivityFilterTypes`).
