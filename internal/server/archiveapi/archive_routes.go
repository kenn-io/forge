...
func (s *Handlers) RegisterArchiveAPI(api huma.API) {
...
func (s *Handlers) listArchivePacing(
	...
	for _, pool := range registry.Snapshot() {
		...
		if window, ok := registry.PacingWindow(pool.Identity, resources); ok {
			...
			status.ResetAt = itemapi.FormatUTCRFC3339(binding.ResetAt)
		}
...
func (s *Handlers) listArchiveStatus(
	ctx context.Context,
...
func (s *Handlers) getArchiveReport(
	ctx context.Context,
...
func archiveQueryRefs(values []string) ([]platform.RepoRef, error) {
	values = activityapi.SplitRepoFilterValues(values)
	refs := make([]platform.RepoRef, 0, len(values))
	for _, value := range values {
		provider, host, _, _, repoPath := itemapi.ParseRepoFilter(value)
...
func (s *Handlers) getArchiveSnapshot(ctx context.Context, input *archiveSnapshotInput) (*archiveSnapshotOutput, error) {
	if s.Archive == nil {
		return nil, httpapi.ServiceUnavailable("archive service not configured")
	}
	start, err := parseArchiveUTCTime("query.start", input.Start)
