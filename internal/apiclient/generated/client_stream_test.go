...
func TestStreamKataTaskEventsReturnsLiveEventStream(t *testing.T) {
	assert := assert.New(t)
	require := require.New(t)
	started := make(chan struct{})
	release := make(chan struct{})
	...
	go func() {
		resp, err := client.StreamKataTaskEvents(context.Background(), nil)
		done <- struct {
			resp *http.Response
			err  error
		}{resp: resp, err: err}
	...
	case result := <-done:
		...
		assert.Equal(": connected\n", line)
	case <-time.After(500 * time.Millisecond):
		require.Fail("StreamKataTaskEvents did not return the live response body")
	}
}
...
func TestGeneratedClientOmitsStreamEventsResponseWrapper(t *testing.T) {
	...
	assert.Contains(clientSource, "StreamKataTaskEvents(ctx context.Context")
	assert.NotContains(clientSource, "StreamKataTaskEventsWithResponse")
	assert.NotContains(clientSource, "ParseStreamKataTaskEventsResponse")
