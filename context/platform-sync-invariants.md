...
- Startup reconciliation reopens legacy GitHub and GitLab archive lookups once when lifecycle details are missing; a GitHub close actor is current only when the latest authored close event matches `closed_at`. (`internal/db/queries_archive.go::RequeueArchiveLifecycleDetails`)
