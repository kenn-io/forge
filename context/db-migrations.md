...
The pre-commit hook `go run ./tools/migrationhistorycheck` compares the base
merge point with the proposed commit index. It rejects edits to migration files
that already exist on `origin/main`, duplicate migration-number assignments, and
branches that introduce more than one new migration version. If a checkout uses
a different main-branch ref, set `MIDDLEMAN_MIGRATION_BASE_REF` before running
the hook.
...
- A branch may introduce only one new migration version. If the branch-local migration is wrong before merge, fold the fix into that same new `.up.sql`/`.down.sql` pair; follow-up repair migrations are for migrations already on `main`.
