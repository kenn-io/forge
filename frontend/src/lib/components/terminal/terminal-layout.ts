...
export function createWorkflowLeaf(
  ...
  return {
    ...
    id,
    tabs: uniqueTabs,
    activeTabKey: uniqueTabs.includes(activeTabKey)
      ? activeTabKey
      : uniqueTabs[0]!,
  };
}
...
export function updateWorkflowSplitRatio(
  ...
  return {
    ...
    second:
      updateWorkflowSplitRatio(node.second, splitID, ratio) ?? node.second,
  };
}
...
export function normalizeTerminalLayout(
  ...
  const activeSessionKey =
    activeFromLayout &&
    selectedGroup?.tree &&
    containsSession(selectedGroup.tree, activeFromLayout)
      ? activeFromLayout
  ...
  return {
    ...
    customSessionLabels: filterRecordByKeys(
      layout.customSessionLabels,
      validKeys,
    ),
  };
}
...
function parseWorkflowNode(value: unknown): WorkflowNode | null {
  ...
  if (node.type === "leaf") {
    ...
    const activeTabKey =
      ...
      isWorkflowTabKey(node.activeTabKey)
        ? node.activeTabKey
        : (tabs[0] ?? "home");
...
function insertWorkflowTabBefore(
  ...
  return {
    ...node,
    first:
      insertWorkflowTabBefore(node.first, sourceTabKey, targetTabKey) ??
      node.first,
    second:
      insertWorkflowTabBefore(node.second, sourceTabKey, targetTabKey) ??
      node.second,
  };
}
...
function insertWorkflowTabIntoLeaf(
  ...
  return {
    ...node,
    first:
      insertWorkflowTabIntoLeaf(node.first, tabKey, leafID, placement) ??
      node.first,
    second:
