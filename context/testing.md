...
`frontend-unit-diagnostics` artifact: it contains Vitest output, `/usr/bin/time`
resource usage, Node fatal reports, and pre/post cgroup memory counters. If the exit
is silent again, rerun it once with repository variable `ACTIONS_RUNNER_DEBUG=true`,
download the runner diagnostic logs, then remove that variable.
