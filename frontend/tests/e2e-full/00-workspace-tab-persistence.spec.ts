...
test.describe("workspace tab persistence", () => {
  ...
  test("shows workspace diff in the right sidebar without adding a stage pane", async ({ page }) => {
    ...
    try {
      ...
      const rightDiffArea = page.locator(".right-sidebar .diff-area .kit-scrollbox__viewport");
      ...
      await expect
        .poll(async () =>
          page.locator(".right-sidebar .diff-area .kit-scrollbox__viewport").evaluate((area) => {
