// Command kenn-forge-github-app creates and manages GitHub Apps whose
// installation tokens kenn-forge uses instead of personal access
...
// kenn-forge's config.toml, where the sync engine picks them up.
package main
...
const usage = `kenn-forge-github-app manages GitHub Apps for kenn-forge's sync engine.

Usage:
  kenn-forge-github-app <command> [flags]

Commands:
  create      Create a GitHub App via the browser manifest flow and
              write its credentials to kenn-forge's config
...
Run "kenn-forge-github-app <command> -h" for command flags.
`
