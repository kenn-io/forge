...
import type { AppRuntime, OwnedAppRuntime } from "../app/runtime.js";
import { makeTestAppRuntime } from "../testing/effect-layers.js";
import { createLogStore as createRuntimeLogStore, type LogStore } from "./roborev/log.svelte.js";
...
async function runLogEffect(program: ReturnType<LogStore["loadSnapshotEffect"]>): Promise<void> {
  if (runtime === undefined) throw new Error("test runtime was not initialized");
...
describe("createLogStore", () => {
  beforeEach(() => {
    runtime = makeTestAppRuntime({} as never);
  });
  ...
  it("loads completed job output snapshots from the JSON response", async () => {
    ...
    expect(requestURL(fetchMock.mock.calls[0]![0])).toBe("http://roborev.test/api/job/output?job_id=42");
  ...
  it("streams live job output from the NDJSON job output endpoint", async () => {
    ...
    expect(requestURL(fetchMock.mock.calls[0]![0])).toBe("http://roborev.test/api/job/output?job_id=77&stream=1");
    expect(requestSignal(fetchMock.mock.calls[0]![0], fetchMock.mock.calls[0]![1])).toBeInstanceOf(AbortSignal);
    expect(store.getLines()).toEqual([
      {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockImplementation((_input, init) => {
      ...
      if (calls === 1) {
        ...
        return new Promise<Response>((_resolve, reject) => {
          staleSignal?.addEventListener("abort", () => reject(new DOMException("request aborted", "AbortError")), {
            once: true,
          });
        });
      }
  ...
  it("does not let stale log teardown stop a successor lease", async () => {
    ...
    if (runtime === undefined) throw new Error("test runtime was not initialized");
    const staleTeardown = runtime.runCommand(store.stopStreamingEffect(firstLease), {
    ...
    const successorTeardown = runtime.runCommand(store.stopStreamingEffect(secondLease), {
  ...
  it("does not start a delayed log lease after that lease was stopped", async () => {
    ...
    const delayedRuntime: AppRuntime = {
      runCommand(program, options) {
        if (options.operation === "stream Roborev job output" && runDelayedStart === undefined) {
          runDelayedStart = () => {
    ...
    const store = createRuntimeLogStore({ baseUrl: "http://roborev.test", runtime: delayedRuntime });
