...
func serveDirectDaemonRequest(t *testing.T, srv *Server, bearer string, headers http.Header) int {
	t.Helper()
...
func TestDirectDaemonBearerHostIntegration(t *testing.T) {
	...
	assert.Equal(t, http.StatusForbidden, serveDirectDaemonRequest(t, srv, "", nil))
	assert.Equal(t, http.StatusForbidden, serveDirectDaemonRequest(t, srv, "wrong", nil))
}
