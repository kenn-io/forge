...
func writeKataProxyCatalog(t *testing.T, home, body string) {
	t.Helper()
...
func writeKataProxyRuntimeRecord(t *testing.T, address string) string {
	t.Helper()
	runtimeDir, err := katacatalog.RuntimeDir()
	require.NoError(t, err)
	require.NoError(t, os.MkdirAll(runtimeDir, 0o700))
	record := katacatalog.RuntimeRecord{
		PID: os.Getpid(), Address: address, StartedAt: time.Now().UTC(),
	}
	body, err := json.Marshal(record)
	require.NoError(t, err)
	path := filepath.Join(runtimeDir, fmt.Sprintf("daemon.%d.json", record.PID))
