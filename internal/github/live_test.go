...
const liveGitHubTestsEnv = "KENN_FORGE_LIVE_GITHUB_TESTS"
...
func liveGitHubToken() string {
	if token := os.Getenv("KENN_FORGE_GITHUB_TOKEN"); token != "" {
		return token
	}
...
func requireLiveGitHubToken(t *testing.T) string {
	...
	require.NotEmpty(t, token, "set KENN_FORGE_GITHUB_TOKEN or GITHUB_TOKEN")
	return token
}
