...
- PR and Issue toggles are independent presentation filters: disabling either hides every attached flat row or threaded item, while repository-level activity remains eligible (`packages/ui/src/components/ActivityFeed.svelte::displayItems`).
- `types` persists item-opening, event, branch, and notification selection; only the fully enabled state omits it, while a fully empty selection uses the nonmatching `none` sentinel (`packages/ui/src/stores/activity.svelte.ts::buildActivityFilterTypes`).
...
- Frontend/store tests: activity type construction, independent item scopes, empty-selection and legacy URL normalization, and notification-row rendering.
