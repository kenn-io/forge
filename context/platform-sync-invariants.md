...
- Inventory calls return one bounded identity page with an advancing opaque cursor or explicit exhaustion, and commit identities with cursor progress atomically. (`internal/platform/types.go::ValidatePage`, `internal/db/queries_archive.go::CommitArchiveInventoryPage`)
...
- Removed, moved, and inaccessible outcomes mark archive work terminal while leaving canonical item content untouched; ambiguous failures remain retryable. (`internal/db/queries_dataset_progress.go::CommitArchiveItemSync`)
