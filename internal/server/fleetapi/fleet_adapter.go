...
func (s *Handler) buildLocalRaw(ctx context.Context) (fleet.RawSnapshot, error) {
	...
	if s.workspaceSnapshot != nil {
		snapshot, err := s.workspaceSnapshot(ctx)
		if err != nil {
			return fleet.RawSnapshot{}, err
		}
...
func nonTmuxRuntimeSessionsForWorkspace(
	...
	for _, si := range s.runtimeSnapshot(sum.ID) {
		if si.TmuxSession != "" {
			continue
		}
...
func nonTmuxRuntimeSessionsForProjectWorktree(
	...
	scope := workspaceapi.ProjectWorktreeRuntimeScope(worktree.ID)
	var out []fleet.RawSession
	for _, si := range s.runtimeSnapshot(scope) {
		if si.TmuxSession != "" {
			continue
		}
		out = append(out, fleet.RawSession{
...
func runtimeSessionByKey(
	...
	for _, si := range s.runtimeSnapshot(workspaceID) {
		out[si.Key] = si
	}
	return out
}
